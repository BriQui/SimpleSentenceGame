...
fun loadSentencesFromDb(db: SQLiteDatabase): List<FlashCard> {
    ...
    try {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                flashCards.add(FlashCard(id, chunkId, sourceSentence, gameSentence, translation, priority, attemptCount))
        ...
        Log.d(DEBUG, "loadSentencesFromDb: loaded ${flashCards.size} rows")

        // Check if no rows were loaded and throw an exception
        if (flashCards.isEmpty()) throw Exception("loadSentencesFromDb: No rows loaded")
    } catch (e: Exception) {
